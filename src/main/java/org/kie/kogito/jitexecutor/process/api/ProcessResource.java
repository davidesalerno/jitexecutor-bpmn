...
public class ProcessResource {
    ...
    public Response listProcess() {
        return Response.ok(new ListIdentifierResponse(jitProcessService.listProcessIdentifiers())).build();
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{processId}/")
    public Response listProcessInstanceByProcessId(@PathParam("processId") String processId) {
        return Response.ok(new ListIdentifierResponse(jitProcessService.listProcessInstanceIdentifiers(processId))).build();
    }

    // creates a process instance for this processId
    @POST
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @Path("/{processId}/")
    public Response startProcessInstanceByProcessId(@PathParam("processId") String processId, StartProcessRequest request) {
        return Response.ok(new IdentifierResponse(jitProcessService.startProcess(processId, request.getParameters()))).build();
    }
